...
public class DistanceFacetsExample {
    ...
    public static Filter getBoundingBoxFilter(double originLat, double originLng, double maxDistanceKM) {
        ...
        f.add(NumericRangeFilter.newDoubleRange("latitude", Math.toDegrees(minLat), Math.toDegrees(maxLat), true, true), BooleanClause.Occur.MUST);

        if (minLng > maxLng) {
            // The bounding box crosses the international date line(国际日期变更线)
            BooleanFilter lonF = new BooleanFilter();
            lonF.add(NumericRangeFilter.newDoubleRange("longitude", Math.toDegrees(minLng), null, true, true), BooleanClause.Occur.SHOULD);
            lonF.add(NumericRangeFilter.newDoubleRange("longitude", null, Math.toDegrees(maxLng), true, true), BooleanClause.Occur.SHOULD);
            f.add(lonF, BooleanClause.Occur.MUST);
        } else {
            f.add(NumericRangeFilter.newDoubleRange("longitude", Math.toDegrees(minLng), Math.toDegrees(maxLng), true, true), BooleanClause.Occur.MUST);
        }
    ...
    public FacetResult search() throws IOException {
        ...
        Facets facets = new DoubleRangeFacetCounts("field", getDistanceValueSource(), fc, getBoundingBoxFilter(ORIGIN_LATITUDE, ORIGIN_LONGITUDE, 10.0), ONE_KM, TWO_KM, FIVE_KM, TEN_KM);
