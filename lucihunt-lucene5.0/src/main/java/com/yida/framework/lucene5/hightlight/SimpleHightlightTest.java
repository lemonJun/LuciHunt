...
public class SimpleHightlightTest {
    ...
    public void testSpanNearQueryHightlighting() throws Exception {
        ...
        IndexReader reader = DirectoryReader.open(ramDir);
        IndexSearcher searcher = new IndexSearcher(reader);
        Query query = new SpanNearQuery(new SpanQuery[] { new SpanTermQuery(new Term(FIELD_NAME, "beginning")), new SpanTermQuery(new Term(FIELD_NAME, "kennedy")) }, 3, false);
    ...
    public void testToParentBlockJoinQuery() throws Exception {
        ...
        Query query = new ToParentBlockJoinQuery(new TermQuery(new Term(FIELD_NAME, "child")), parentFilter, ScoreMode.Total);
        hits = searcher.search(query, 100);
        int maxNumFragmentsRequired = 3;
        ...
        for (int i = 0; i < hits.totalHits; i++) {
            ...
            String fragment = highlighter.getBestFragments(tokenStream, text, maxNumFragmentsRequired, "...");
