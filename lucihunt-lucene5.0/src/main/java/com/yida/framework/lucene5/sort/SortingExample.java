...
public class SortingExample {
    ...
    public void displayResults(Query query, Sort sort) throws IOException {
        ...
        for (ScoreDoc sd : results.scoreDocs) {
            ...
            out.println(StringUtils.rightPad(StringUtils.abbreviate(doc.get("title"), 29), 30) + StringUtils.rightPad(doc.get("pubmonth"), 10) + StringUtils.center("" + docID, 4) + StringUtils.leftPad(scoreFormatter.format(score), 12));
    ...
    public static void main(String[] args) throws Exception {
        ...
        example.displayResults(query, new Sort(new SortField("pubmonth", Type.INT, true)));

        example.displayResults(query, new Sort(new SortField("category", Type.STRING), SortField.FIELD_SCORE, new SortField("pubmonth", Type.INT, true)));

        example.displayResults(query, new Sort(new SortField[] { SortField.FIELD_SCORE, new SortField("category", Type.STRING) }));
        directory.close();
    }
}