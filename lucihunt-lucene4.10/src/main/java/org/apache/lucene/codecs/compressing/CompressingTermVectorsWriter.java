...
public final class CompressingTermVectorsWriter extends TermVectorsWriter {
    ...
    private void flushFlags(int totalFields, int[] fieldNums) throws IOException {
        ...
        outer: for (DocData dd : pendingDocs) {
            for (FieldData fd : dd.fields) {
    ...
    private void flushNumTerms(int totalFields) throws IOException {
        ...
        final PackedInts.Writer writer = PackedInts.getWriterNoHeader(vectorsStream, PackedInts.Format.PACKED, totalFields, bitsRequired, 1);
        for (DocData dd : pendingDocs) {
            for (FieldData fd : dd.fields) {
    ...
    public int merge(MergeState mergeState) throws IOException {
        ...
        for (AtomicReader reader : mergeState.readers) {
            ...
            if (matchingVectorsReader == null || matchingVectorsReader.getVersion() != VERSION_CURRENT || matchingVectorsReader.getCompressionMode() != compressionMode || matchingVectorsReader.getChunkSize() != chunkSize || matchingVectorsReader.getPackedIntsVersion() != PackedInts.VERSION_CURRENT) {
                // naive merge...
                ...
                for (int i = nextLiveDoc(0, liveDocs, maxDoc); i < maxDoc;) {
                    ...
                    if (pendingDocs.isEmpty() && (i == 0 || index.getStartPointer(i - 1) < startPointer)) { // start of a chunk
