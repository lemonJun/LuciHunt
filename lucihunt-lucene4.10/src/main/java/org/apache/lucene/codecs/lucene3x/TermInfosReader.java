...
class TermInfosReader implements Closeable {
    ...
    TermInfosReader(Directory dir, String seg, FieldInfos fis, IOContext context, int indexDivisor) throws CorruptIndexException, IOException {
        ...
        try {
            ...
            origEnum = newSegmentTermEnum(directory.openInput(IndexFileNames.segmentFileName(segment, "", Lucene3xPostingsFormat.TERMS_EXTENSION), context), fieldInfos, false);
            size = origEnum.size;
            ...
            if (indexDivisor != -1) {
                ...
                final SegmentTermEnum indexEnum = newSegmentTermEnum(directory.openInput(indexFileName, context), fieldInfos, true);
    ...
    TermInfo seekEnum(SegmentTermEnum enumerator, Term term, TermInfoAndOrd tiOrd, boolean useCache) throws IOException {
        ...
        if (enumerator.term() != null // term is at or past current
                        && ((enumerator.prev() != null && compareAsUTF16(term, enumerator.prev()) > 0) || compareAsUTF16(term, enumerator.term()) >= 0)) {
