...
class FieldCacheImpl implements FieldCache {
    ...
    abstract static class Cache {
        ...
        protected abstract Accountable createValue(AtomicReader reader, CacheKey key, boolean setDocsWithField) throws IOException;
    ...
    static final class DocsWithFieldCache extends Cache {
        ...
        @Override
        protected Accountable createValue(AtomicReader reader, CacheKey key, boolean setDocsWithField /* ignored */) throws IOException {
    ...
    @Override
    public Longs getLongs(AtomicReader reader, String field, FieldCache.LongParser parser, boolean setDocsWithField) throws IOException {
        final NumericDocValues valuesIn = reader.getNumericDocValues(field);
        if (valuesIn != null) {
            // Not cached here by FieldCacheImpl (cached instead
            // per-thread by SegmentReader):
    ...
    @Override
    public Doubles getDoubles(AtomicReader reader, String field, FieldCache.DoubleParser parser, boolean setDocsWithField) throws IOException {
        final NumericDocValues valuesIn = reader.getNumericDocValues(field);
        if (valuesIn != null) {
            // Not cached here by FieldCacheImpl (cached instead
            // per-thread by SegmentReader):
    ...
    public static class SortedDocValuesImpl implements Accountable {
        ...
        @Override
        public long ramBytesUsed() {
            return 3 * RamUsageEstimator.NUM_BYTES_OBJECT_REF + RamUsageEstimator.NUM_BYTES_INT + bytes.ramBytesUsed() + termOrdToBytesOffset.ramBytesUsed() + docToTermOrd.ramBytesUsed();
        }
    }
