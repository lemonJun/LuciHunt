...
public abstract class SimilarityBase extends Similarity {
    ...
    protected Explanation explain(BasicStats stats, int doc, Explanation freq, float docLen) {
        ...
        result.setDescription("score(" + getClass().getSimpleName() + ", doc=" + doc + ", freq=" + freq.getValue() + "), computed from:");
    ...
    private class BasicSimScorer extends SimScorer {
        ...
        public float score(int doc, float freq) {
            ...
            return SimilarityBase.this.score(stats, freq, norms == null ? 1F : decodeNormValue((byte) norms.get(doc)));
        }

        @Override
        public Explanation explain(int doc, Explanation freq) {
            return SimilarityBase.this.explain(stats, doc, freq, norms == null ? 1F : decodeNormValue((byte) norms.get(doc)));
        }
