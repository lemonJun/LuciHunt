...
class DocumentsWriterPerThread {
    ...
    public DocumentsWriterPerThread(String segmentName, Directory directory, LiveIndexWriterConfig indexWriterConfig, InfoStream infoStream, DocumentsWriterDeleteQueue deleteQueue, FieldInfos.Builder fieldInfos, AtomicLong pendingNumDocs) throws IOException {
    ...
    FlushedSegment flush() throws IOException {
        ...
        final SegmentWriteState flushState = new SegmentWriteState(infoStream, directory, segmentInfo, fieldInfos.finish(), indexWriterConfig.getTermIndexInterval(), pendingUpdates, new IOContext(new FlushInfo(numDocsInRAM, bytesUsed())));
        ...
        try {
            ...
            if (infoStream.isEnabled("DWPT")) {
                ...
                infoStream.message("DWPT", "flushed: segment=" + segmentInfo.name + " ramUsed=" + nf.format(startMBUsed) + " MB" + " newFlushedSize(includes docstores)=" + nf.format(newSegmentSize) + " MB" + " docs/MB=" + nf.format(flushState.segmentInfo.getDocCount() / newSegmentSize));
            }
