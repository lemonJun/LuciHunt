...
final class DocumentsWriterFlushControl implements Accountable {
    ...
    void markForFullFlush() {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            try {
                ...
                assert next.dwpt.deleteQueue == flushingQueue || next.dwpt.deleteQueue == documentsWriter.deleteQueue : " flushingQueue: " + flushingQueue + " currentqueue: " + documentsWriter.deleteQueue + " perThread queue: " + next.dwpt.deleteQueue + " numDocsInRam: " + next.dwpt.getNumDocsInRAM();
    ...
    void waitIfStalled() {
        if (infoStream.isEnabled("DWFC")) {
            infoStream.message("DWFC", "waitIfStalled: numFlushesPending: " + flushQueue.size() + " netBytes: " + netBytes() + " flushBytes: " + flushBytes() + " fullFlush: " + fullFlush);
        }
