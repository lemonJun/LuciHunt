...
public class TestRecyclingIntBlockAllocator extends LuceneTestCase {
    ...
    private RecyclingIntBlockAllocator newAllocator() {
        return new RecyclingIntBlockAllocator(1 << (2 + random().nextInt(15)), random().nextInt(97), Counter.newCounter());
    }
    ...
    public void testAllocateAndFree() {
        ...
        for (int i = 0; i < numIters; i++) {
            ...
            for (int j = 0; j < num; j++) {
                ...
                assertEquals(size, block.length);
                assertTrue("block is returned twice", allocated.add(block));
                assertEquals("" + (4 * size * (allocated.size() + allocator.numBufferedBlocks()) - allocator.bytesUsed()), 4 * size * (allocated.size() + allocator.numBufferedBlocks()), allocator.bytesUsed());
            }
