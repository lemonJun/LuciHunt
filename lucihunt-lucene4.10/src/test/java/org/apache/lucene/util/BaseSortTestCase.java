...
public abstract class BaseSortTestCase extends LuceneTestCase {
    ...
    enum Strategy {
        ...
        ASCENDING {
            @Override
            public void set(Entry[] arr, int i) {
                arr[i] = i == 0 ? new Entry(random().nextInt(6), 0) : new Entry(arr[i - 1].value + random().nextInt(6), i);
            }
        },
        DESCENDING {
            @Override
        ...
        STRICTLY_DESCENDING {
            @Override
            public void set(Entry[] arr, int i) {
                arr[i] = i == 0 ? new Entry(random().nextInt(6), 0) : new Entry(arr[i - 1].value - TestUtil.nextInt(random(), 1, 5), i);
            }
        },
        ASCENDING_SEQUENCES {
            @Override
            public void set(Entry[] arr, int i) {
                arr[i] = i == 0 ? new Entry(random().nextInt(6), 0) : new Entry(rarely() ? random().nextInt(1000) : arr[i - 1].value + random().nextInt(6), i);
            }
        },
        MOSTLY_ASCENDING {
            @Override
            public void set(Entry[] arr, int i) {
                arr[i] = i == 0 ? new Entry(random().nextInt(6), 0) : new Entry(arr[i - 1].value + TestUtil.nextInt(random(), -8, 10), i);
            }
        };
