...
public class TestIndexWriterCommit extends LuceneTestCase {
    ...
    public void testCommitOnCloseForceMerge() throws IOException {
        ...
        IndexWriter writer = new IndexWriter(dir, newIndexWriterConfig(new MockAnalyzer(random())).setMaxBufferedDocs(10).setMergePolicy(newLogMergePolicy(10)));
        for (int j = 0; j < 17; j++) {
            TestIndexWriter.addDocWithIndex(writer, j);
        }
        writer.close();
    ...
    public void testCommitUserData() throws IOException {
        ...
        w = new IndexWriter(dir, newIndexWriterConfig(new MockAnalyzer(random())).setMaxBufferedDocs(2));
        for (int j = 0; j < 17; j++)
            TestIndexWriter.addDoc(w);
