...
public class TestThreadedForceMerge extends LuceneTestCase {
    ...
    public void runTest(Random random, Directory directory) throws Exception {
        ...
        IndexWriter writer = new IndexWriter(directory, newIndexWriterConfig(ANALYZER).setOpenMode(OpenMode.CREATE).setMaxBufferedDocs(2).setMergePolicy(newLogMergePolicy()));
        ...
        for (int iter = 0; iter < NUM_ITER; iter++) {
            ...
            writer.close();
            writer = new IndexWriter(directory, newIndexWriterConfig(ANALYZER).setOpenMode(OpenMode.APPEND).setMaxBufferedDocs(2));
