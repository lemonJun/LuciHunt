...
public class TestIndexWriterReader extends LuceneTestCase {
            ...
            if (i == 0) {
                ...
                switch (random().nextInt(5)) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        writer.updateDocument(new Term("id", "" + previous), DocHelper.createDocument(previous, "x", 1 + random().nextInt(5)));
                        break;
                    case 4:
    ...
    public void testDeleteFromIndexWriter() throws Exception {
        ...
        Directory dir1 = getAssertNoDeletesDirectory(newDirectory());
        IndexWriter writer = new IndexWriter(dir1, newIndexWriterConfig(new MockAnalyzer(random())).setReaderTermsIndexDivisor(2));
        // create the index
        createIndexNoClose(!doFullMerge, "index1", writer);
        writer.flush(false, true);
        // get a reader
    ...
    public void testAddIndexesAndDoDeletesThreads() throws Throwable {
        ...
        IndexWriter mainWriter = new IndexWriter(mainDir, newIndexWriterConfig(new MockAnalyzer(random())).setMergePolicy(newLogMergePolicy()));
    ...
    private class AddDirectoriesThreads {
        ...
        public AddDirectoriesThreads(int numDirs, IndexWriter mainWriter) throws Throwable {
            ...
            addDir = newDirectory();
            IndexWriter writer = new IndexWriter(addDir, newIndexWriterConfig(new MockAnalyzer(random())).setMaxBufferedDocs(2));
    ...
    public static void createIndex(Random random, Directory dir1, String indexName, boolean multiSegment) throws IOException {
    ...
    public static void createIndexNoClose(boolean multiSegment, String indexName, IndexWriter w) throws IOException {
        for (int i = 0; i < 100; i++) {
            w.addDocument(DocHelper.createDocument(i, indexName, 4));
        }
        if (!multiSegment) {
    ...
    public void testAfterCommit() throws Exception {
        Directory dir1 = getAssertNoDeletesDirectory(newDirectory());
        IndexWriter writer = new IndexWriter(dir1, newIndexWriterConfig(new MockAnalyzer(random())).setMergeScheduler(new ConcurrentMergeScheduler()));
        writer.commit();
    ...
    public void testNoTermsIndex() throws Exception {
        ...
        IndexWriterConfig conf = newIndexWriterConfig(new MockAnalyzer(random())).setReaderTermsIndexDivisor(-1);

        // Don't proceed if picked Codec is in the list of illegal ones.
        final String format = TestUtil.getPostingsFormat("f");
        assumeFalse("Format: " + format + " does not support ReaderTermsIndexDivisor!", (format.equals("FSTPulsing41") || format.equals("FSTOrdPulsing41") || format.equals("FST41") || format.equals("OrdsLucene41") || format.equals("FSTOrd41") || format.equals("SimpleText") || format.equals("Memory") || format.equals("MockRandom") || format.equals("Direct")));
