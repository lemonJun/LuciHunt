...
public class TestDocumentsWriterDeleteQueue extends LuceneTestCase {
    ...
    private void assertAllBetween(int start, int end, BufferedUpdates deletes, Integer[] ids) {
        for (int i = start; i <= end; i++) {
            assertEquals(Integer.valueOf(end), deletes.terms.get(new Term("id", ids[i].toString())));
        }
    }
    ...
    private static class UpdateThread extends Thread {
        ...
        protected UpdateThread(DocumentsWriterDeleteQueue queue, AtomicInteger index, Integer[] ids, CountDownLatch latch) {
            this.queue = queue;
            this.index = index;
            this.ids = ids;
