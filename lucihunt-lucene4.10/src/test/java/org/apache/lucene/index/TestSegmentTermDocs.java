    ...
    public void testBadSeek(int indexDivisor) throws IOException {
        {
            ...
            SegmentReader reader = new SegmentReader(info, indexDivisor, newIOContext(random()));
            assertTrue(reader != null);
            DocsEnum termDocs = TestUtil.docs(random(), reader, "textField2", new BytesRef("bad"), reader.getLiveDocs(), null, 0);
        ...
        }
        {
            ...
            SegmentReader reader = new SegmentReader(info, indexDivisor, newIOContext(random()));
            assertTrue(reader != null);
            DocsEnum termDocs = TestUtil.docs(random(), reader, "junk", new BytesRef("bad"), reader.getLiveDocs(), null, 0);
            assertNull(termDocs);
            reader.close();
        }
    }
