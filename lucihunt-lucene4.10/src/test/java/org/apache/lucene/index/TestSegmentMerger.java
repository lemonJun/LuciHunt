...
public class TestSegmentMerger extends LuceneTestCase {
    ...
    public void testMerge() throws IOException {
        ...
        SegmentMerger merger = new SegmentMerger(Arrays.<AtomicReader> asList(reader1, reader2), si, InfoStream.getDefault(), mergedDir, IndexWriterConfig.DEFAULT_TERM_INDEX_INTERVAL, MergeState.CheckAbort.NONE, new FieldInfos.FieldNumbers(), newIOContext(random()), true);
        MergeState mergeState = merger.merge();
        int docsMerged = mergeState.segmentInfo.getDocCount();
        assertTrue(docsMerged == 2);
        //Should be able to open a new SegmentReader against the new directory
        SegmentReader mergedReader = new SegmentReader(new SegmentCommitInfo(new SegmentInfo(mergedDir, Version.LATEST, mergedSegment, docsMerged, false, codec, null), 0, -1L, -1L, -1L), DirectoryReader.DEFAULT_TERMS_INDEX_DIVISOR, newIOContext(random()));
