...
public class TestFlushByRamOrCountsPolicy extends LuceneTestCase {
    ...
    protected void runFlushByRam(int numThreads, double maxRamMB, boolean ensureNotStalled) throws IOException, InterruptedException {
    ...
    private static class MockDefaultFlushPolicy extends FlushByRamOrCountsPolicy {
        ...
        public void onDelete(DocumentsWriterFlushControl control, ThreadState state) {
            ...
            if (state.flushPending) {
                toFlush = state;
            } else if (flushOnDeleteTerms() && state.dwpt.pendingUpdates.numTermDeletes.get() >= indexWriterConfig.getMaxBufferedDeleteTerms()) {
                toFlush = state;
            } else {
                toFlush = null;
            }
    ...
    static void findPending(DocumentsWriterFlushControl flushControl, ArrayList<ThreadState> pending, ArrayList<ThreadState> notPending) {
        Iterator<ThreadState> allActiveThreads = flushControl.allActiveThreadStates();
        while (allActiveThreads.hasNext()) {
            ThreadState next = allActiveThreads.next();
