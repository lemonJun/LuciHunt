...
public class IndexWriter {
    ...
    private final void flushRamSegments() throws IOException {
        ...
        if (minSegment < 0 || // add one FS segment?
                        (docCount + segmentInfos.info(minSegment).docCount) > mergeFactor || !(segmentInfos.info(segmentInfos.size() - 1).dir == ramDirectory))
            minSegment++;
    ...
    private final void mergeSegments(int minSegment, int end) throws IOException {
        ...
        for (int i = minSegment; i < end; i++) {
            ...
            merger.add(reader);
            // if we own the directory  准备合并完删掉
