...
public class TestMultiSearcherRanking extends TestCase {
    ...
    */
    ...
    private void checkQuery(String queryStr) throws IOException, ParseException {
        ...
        Query query = QueryParser.parse(queryStr, FIELD_NAME, new StandardAnalyzer());
        ...
        for (int i = 0; i < multiSearcherHits.length(); i++) {
            ...
            if (verbose)
                System.out.println("Multi:  " + docMulti.get(FIELD_NAME) + " score=" + multiSearcherHits.score(i));
            if (verbose)
                System.out.println("Single: " + docSingle.get(FIELD_NAME) + " score=" + singleSearcherHits.score(i));
