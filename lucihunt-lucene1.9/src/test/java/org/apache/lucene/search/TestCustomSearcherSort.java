...
public class TestCustomSearcherSort extends TestCase implements Serializable {
    ...
    public void testFieldSortSingleSearcher() throws Exception {
        ...
        // define the sort criteria
        Sort custSort = new Sort(new SortField[] { new SortField("publicationDate_"), SortField.FIELD_SCORE });
        Searcher searcher = new MultiSearcher(new Searchable[] { new CustomSearcher(index, 2) });
        // search and check hits
        matchHits(searcher, custSort);
    }
    ...
    public void testFieldSortMultiCustomSearcher() throws Exception {
        ...
        // define the sort criteria
        Sort custSort = new Sort(new SortField[] { new SortField("publicationDate_"), SortField.FIELD_SCORE });
        Searcher searcher = new MultiSearcher(new Searchable[] { new CustomSearcher(index, 0), new CustomSearcher(index, 2) });
        // search and check hits
        matchHits(searcher, custSort);
    }
    ...
    private void matchHits(Searcher searcher, Sort sort) throws IOException {
        ...
        for (int hitid = 0; hitid < hitsByRank.length(); ++hitid) {
            resultMap.put(new Integer(hitsByRank.id(hitid)), // Key:   Lucene Document ID
