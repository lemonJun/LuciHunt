...
class AnalysisTest {
    public static void main(String[] args) {
        try {
            test("English is a West Germanic language that was first spoken in early medieval England and", true);
    ...
    static void test(File file, boolean verbose) throws Exception {
    ...
    static void test(Reader reader, boolean verbose, long bytes) throws Exception {
        ...
        for (Token t = stream.next(); t != null; t = stream.next()) {
            if (verbose) {
                System.out.println("Text=" + t.termText() + " start=" + t.startOffset() + " end=" + t.endOffset());
            }
            count++;
        }
