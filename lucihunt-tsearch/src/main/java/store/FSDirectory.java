...
import index.IndexFileNameFilter;
...
public class FSDirectory extends Directory {
     ...
     */
    private static final WeakHashMap<File, FSDirectory> DIRECTORIES = new WeakHashMap<File, FSDirectory>();

    private static volatile boolean disableLocks = false;
    ...
    /** 
     * 返回一个指定位置的directory类
     * 并且此数据会被缓存，对于一个指定的路径 ，会始终返回这一个DIRECTORY
     * Returns the directory instance for the named location.
     *
     * <p>Directories are cached, so that, for a given canonical path, the same
     * FSDirectory instance will always be returned.  This permits
    ...
    //如果需要新创建  此一步会清除 此目录下的所有已有的LUCENE文件
     ...
     */
    public FileLock makeLock(String name) {
