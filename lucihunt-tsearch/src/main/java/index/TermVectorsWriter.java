 ...
 *
 * @author WangYazhou
 * @date  2017年4月13日 下午5:59:29
 * @see
 */
public class TermVectorsWriter {
    ...
    private Vector<TVField> fields;
    private Vector<TVTerm> terms;

    private long currentDocPointer = -1L;

    private TVField currentField;
    ...
    public TermVectorsWriter(Directory directory, String segment, FieldInfos fieldInfos) throws IOException {
        ...
        tvx.writeInt(FORMAT_VERSION);//都是先写版本信息
        tvd.writeInt(FORMAT_VERSION);//都是先写版本信息
        tvf.writeInt(FORMAT_VERSION);//都是先写版本信息

        fieldinfos = fieldInfos;
        fields = new Vector<TVField>(fieldInfos.size());
        terms = new Vector<TVTerm>();
    }
    ...
    private void writeField() throws IOException {
        ...
        //3 一个字节  最后一位是否存储位置信息   倒数第二是否保存偏移量                                                                                                                                                                                                                                          
        tvf.writeByte(bits);
